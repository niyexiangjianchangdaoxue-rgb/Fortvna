...
def test_uuid7_rfc9562_appendix_b2_known_answer() -> None:
...
def test_uuid7_max_fields_isolate_version_variant() -> None:
...
def test_uuid7_timestamp_roundtrip() -> None:
...
def test_uuid7_rejects_out_of_range(ts: int, ra: int, rb: int) -> None:
    with pytest.raises(ValueError):
        ids.uuid7(ts, ra, rb)


def test_generate_uuid7_injected() -> None:
    class FakeClock:
...
def test_secrets_rng_within_range() -> None:
    rng = ids.SecretsRng()
    for _ in range(1000):
